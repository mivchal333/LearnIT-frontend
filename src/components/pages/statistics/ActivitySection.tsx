...
const ActivitySection = () => {
    ...
    const getData = (userHistoryEntries: UserHistoryEntries) => {
        let mapData: { [date: number]: { correctCount: number, badCount: number } } = {}
        ...
        values(userHistoryEntries).flatMap(userAttempts => userAttempts)
            .map(userAttempt => {
                ...
                userAttempt.history.forEach(answer => {
                    const chartEntry = mapData[attemptDateValueOf] || {}
                    if (answer.answerResult) {
                        const count = chartEntry.correctCount || 0;
                        mapData[attemptDateValueOf] = {
                            ...chartEntry,
                            correctCount: count + 1
                        }

                    } else {
                        const count1 = chartEntry.badCount || 0;
                        mapData[attemptDateValueOf] = {
                            ...chartEntry,
                            badCount: count1 + 1
                        }
        ...
        <Paper className={classes.paper}>
            ...
            <ResponsiveContainer width="100%" height="100%">
                ...
                >
                    ...
                    <Tooltip/>
                    <Legend/>
                    <Bar dataKey="correctCount" fill={colors.green["400"]} name="Poprawne odpowiedzi"/>
                    <Bar dataKey="badCount" fill={colors.red["400"]} name="Niepoprawne odpowiedzi"/>
                </BarChart>
            </ResponsiveContainer>
        </Paper>
    )
